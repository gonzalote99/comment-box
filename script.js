...
var createCommentStructure = function (target, header, content, upvote, date) {
  ...
  downVoteBtn.classList = 'downvote-btn';
  ...
  downVoteBtn.addEventListener('click', function () {
    var upvote = parseInt(commentFeature.getAttribute('date-vote') || 0);
    commentFeature.setAttribute('date-vote', upvote === 0 ? 0 : --upvote);
    upvoteCount.innerText = upvote;
  }, false);
  ...
  replyBtn.addEventListener('click', function (e) {
    ...
    if (regex.test(inputElement.className)) {
      inputElement.classList = 'comment-box reply-comment hide';

    } else {
      inputElement.classList = 'comment-box reply-comment show';
      inputElement.focus();
    }
  }, false);
  ...
  newComment.classList = 'new-comment';
